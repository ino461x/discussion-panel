...
## 例: 「チャット機能のリファクタリング」がトピックの場合

**Critic への Starting Artifact 指示:**
> 3つの失敗シナリオを思考せよ。特に「履歴フィルタが常に正しく機能する」という前提が
> 間違っていたらどうなるかから始めよ。

**Realist への Starting Artifact 指示:**
> フェーズ別の人日見積もりを思考せよ。特に「ハンドラのステージ分割」の実装コストと
> リグレッション検証コストを優先せよ。

**Architect への Starting Artifact 指示:**
> 依存関係のチェーンを思考せよ。特にメインのサービスモジュールを起点として、
> 複数のエントリポイント (HTTP / webhook / 外部API) がどこで分岐し収束するかをマッピングせよ。

**Contrarian への Starting Artifact 指示:**
> ユーザーの核心的主張「統合により複雑度が下がる」が提案されなかった世界を想像せよ。
